...
vector<GraphVertex*> Graph::searchDFS(int start) {
	...
	stack.push(edgeArray[start].first);	// correct so far
	...
	while (!stack.empty()) {
		...
		for (int i = 0; i < edgeArray[u->Value].second->size(); i++) {
			stack.push(edgeArray[u->Value].second->at(i));
		}
	}
...
vector<GraphVertex*> Graph::searchBFS(int start) {
	...
	queue.push(edgeArray[start].first);	// correct so far
	...
	while (!queue.empty()) {
		...
		returnVector.push_back(u);
		for (int i = 0; i < edgeArray[u->Value].second->size(); i++) {
			queue.push(edgeArray[u->Value].second->at(i));
		}
	}
